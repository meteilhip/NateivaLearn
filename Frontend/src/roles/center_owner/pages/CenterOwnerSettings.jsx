...
export function CenterOwnerSettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.button
        ...
        onClick={() => navigate("/center_owner/profile")}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow-sm divide-y divide-black/10">
        {SECTIONS.map((s) => (
          <button key={s.key} type="button" onClick={() => s.path && navigate(s.path)} className="w-full flex items-center gap-4 px-6 py-4 text-left hover:bg-black/5 transition">
            <s.icon className="text-primary w-5 h-5 flex-shrink-0" />
            <span className="font-medium text-dark">{t(`settings.${s.key}`)}</span>
            {s.path && <span className="ml-auto text-dark/50">→</span>}
          </button>
        ))}
      </motion.div>
    </div>
